...
export default function MakeupProductDetails() {
      ...
      try {
        ...
        if (data.brand) {
          const relatedResponse = await fetch(
            `https://makeup-api.herokuapp.com/api/v1/products.json?brand=${data.brand}&product_type=${data.product_type}`
          );
      ...
      <div className="grid md:grid-cols-2 gap-8 mt-6">
        ...
        <div>
          ...
          <div className="flex gap-2 overflow-x-auto">
            {[product.image_link, ...(product.product_colors || []).map(color => color.hex_value)].map((img, index) => (
              <button
                key={index}
